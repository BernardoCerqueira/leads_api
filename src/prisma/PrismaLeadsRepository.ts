...
import { CreateLeadAttributes, FindLeadsParams, LeadsRepository, LeadWhereParams } from "../repositories/LeadsRepository";
...
export class PrismaLeadsRepository implements LeadsRepository { 
    async find(params: FindLeadsParams): Promise<Lead[]>{
        ...
        if(params.where?.campaignId){
            where.campaigns = {some: {campaingId: params.where.campaignId}}
        }
    ...
    async count(where: LeadWhereParams): Promise<number> {
        ...
        if(where?.campaignId){
            prismaWhere.campaigns = {some: {campaingId: where.campaignId}}
        }
